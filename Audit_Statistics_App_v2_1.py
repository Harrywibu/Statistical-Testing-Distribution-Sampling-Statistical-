...
st.set_page_config(page_title='Audit Statistics — v2.1.6 Patched (PLUS)', layout='wide')

# --- Safe wrapper to avoid DuplicateElementId on Plotly charts ---
try:
    _ = st.session_state
except Exception:
    pass
...
def st_plotly(fig, **kwargs):
    """Wrapper over st.plotly_chart that injects a unique key if missing,
    and defaults width='stretch' to adhere to new Streamlit API."""
    SS = st.session_state
...
st.title('📊 Audit Statistics — v2.1.6 Patched (PLUS)')
...
TAB0, TAB1, TAB2, TAB3, TAB4, TAB5, TAB6, TAB7 = st.tabs([
    '0) Population Analysis', '1) Profiling', '2) Trend & Correlation', '3) Benford 1D/2D', '4) Tests', '5) Regression', '6) Fraud Flags', '7) Risk & Export'
])
...
def recommend_tests_from_selection(df, selected_cols, signals):
    recs = []
    nums = [c for c in selected_cols if c in df.columns and pd.api.types.is_numeric_dtype(df[c])]
    cats = [c for c in selected_cols if c in df.columns and not pd.api.types.is_numeric_dtype(df[c])]
    # One numeric + one categorical
    for y in nums:
        for g in cats:
            if y != g and df[g].nunique(dropna=True) >= 2:
                if (signals.get('normality_p', 1.0) < 0.05) or (abs(signals.get('skew',0))>1) or (abs(signals.get('kurtosis',0))>3):
                    recs.append(f"[{y} ~ {g}] → Kruskal–Wallis/Mann–Whitney (non-parametric)")
                else:
                    recs.append(f"[{y} ~ {g}] → ANOVA (kèm Levene)")
    # Two numeric
    if len(nums) >= 2:
        recs.append("Hai biến numeric → Pearson/Spearman; nếu mục tiêu dự báo → Regression")
    # Binary flag + categorical
    bin_cols = []
    ...
    for f in bin_cols:
        for g in cats:
            if g != f and df[g].nunique(dropna=True) >= 2:
                recs.append(f"[{f} by {g}] → Proportion χ² (hoặc Fisher nếu ô thưa)")
    # Datetime → Trend
    dt_cols = [c for c in selected_cols if c in df.columns and pd.api.types.is_datetime64_any_dtype(df[c])]
    if dt_cols:
        recs.append("Có datetime → Trend (D/W/M/Q + Rolling/YoY); nếu spike cuối kỳ → test cut‑off")
    # Signals
    if signals.get('zero_ratio',0) > 0.3:
        recs.append("Zero‑heavy → kiểm tra tỷ lệ 0 theo nhóm (χ²/Fisher), soát policy/threshold")
    if signals.get('tail_gt_p99_share',0) > 0.02:
        recs.append("Tail>p99 > 2% → Benford 1D/2D + cut‑off cuối kỳ + drill‑down entity/staffer")
    if (signals.get('benford_p') is not None and signals['benford_p'] < 0.05) or (signals.get('benford_MAD',0) > 0.012):
        recs.append("Benford lệch (p<0.05 hoặc MAD>0.012) → drill‑down chữ số lệch + outlier review + policy check")
    return recs

# ---------- TAB 0: Population Analysis (Module options) ----------
with TAB0:
    st.subheader('📐 Population Analysis — Central Tendency, Shape, Factor & Sampling Error')
    c1, c2, c3 = st.columns(3)
    with c1:
        num_col0 = st.selectbox('Numeric column', num_cols or df.columns.tolist(), key='pa_num')
    with c2:
        group_col0 = st.selectbox('Group (optional, categorical)', ['(None)'] + cat_cols, key='pa_grp')
    with c3:
        dt_col0 = st.selectbox('Datetime (optional)', ['(None)'] + df.columns.tolist(), key='pa_dt')

    if num_col0:
        s0 = pd.to_numeric(df[num_col0], errors='coerce').replace([np.inf,-np.inf], np.nan)
        ...
        if len(s)==0:
            ...
            # Sampling Error & 95% CI for mean
            se = float(s.std(ddof=1)/np.sqrt(len(s))) if len(s)>1 else np.nan
            ci_l = float(s.mean() - 1.96*se) if not np.isnan(se) else np.nan
            ci_u = float(s.mean() + 1.96*se) if not np.isnan(se) else np.nan

            stat_df = pd.DataFrame([{
                'count': int(desc['count']), 'n_missing': n_na, 'mean': desc['mean'], 'std': desc['std'], 'SE_mean': se,
            ...
            # Shape of curve
            st.markdown('**Shape of Curve (Distribution)**')
            if HAS_PLOTLY:
                g1,g2 = st.columns(2)
                with g1:
                    ...
                    fig1.update_layout(title=f'{num_col0} — Histogram+KDE', height=320)
                    st_plotly(fig1); register_fig('Population', f'{num_col0} — Histogram+KDE', fig1, 'Hình dạng phân phối & đuôi.')
                    st.caption('**Ý nghĩa**: Nhìn shape, lệch, đa đỉnh; KDE giúp mượt mật độ.')
                with g2:
                    fig2 = px.box(pd.DataFrame({num_col0:s}), x=num_col0, points='outliers', title=f'{num_col0} — Box')
                    st_plotly(fig2); register_fig('Population', f'{num_col0} — Box', fig2, 'Trung vị/IQR; outliers.')
                    st.caption('**Ý nghĩa**: Trung vị & IQR; điểm bật ra là ứng viên ngoại lệ.')
            g3,g4 = st.columns(2)
            with g3:
                try:
                    fig3 = px.ecdf(s, title=f'{num_col0} — ECDF')
                    st_plotly(fig3); register_fig('Population', f'{num_col0} — ECDF', fig3, 'P(X≤x) tích luỹ.')
            ...
            with g4:
                try:
                    ...
                    fig4.update_layout(title=f'{num_col0} — QQ Normal', height=320)
                    st_plotly(fig4); register_fig('Population', f'{num_col0} — QQ Normal', fig4, 'Độ lệch so với normal.')
            ...
            # Factor Analysis (light): overlay by group if provided
            st.markdown('**Factor Analysis (by Group)**')
            grp = None if group_col0=='(None)' else group_col0
            if grp and grp in df.columns and df[grp].nunique(dropna=True)>=2:
                sub = df[[num_col0, grp]].dropna()
                if not sub.empty and HAS_PLOTLY:
                    figG = px.box(sub, x=grp, y=num_col0, color=grp, title=f'{num_col0} by {grp}')
                    st_plotly(figG); register_fig('Population', f'{num_col0} by {grp}', figG, 'So sánh theo nhóm.')
                # quick suggestion: ANOVA vs Kruskal based on normality/skew
                st.caption('**Gợi ý**: Nếu normality kém/skew cao → Kruskal–Wallis; ngược lại thử ANOVA (kèm Levene).')
            else:
                st.caption('Chọn một cột nhóm (categorical, ≥2 nhóm) để so sánh theo nhóm.')

            # Auto Recommend Tests based on signals
            signals = {
                'skew': skew,
                'kurtosis': kurt,
                'normality_p': p_norm,
                'zero_ratio': float((s==0).mean()),
                'tail_gt_p99_share': float((s>p99).mean())
            }
            recs = recommend_tests_from_selection(df, [num_col0, grp, None if dt_col0=='(None)' else dt_col0], signals)
            st.markdown('### 🧭 Recommend Tests (Auto)')
            st.write('\n'.join([f'- {x}' for x in recs]) if recs else '- Không có đề xuất đặc biệt.')

# ---------- TAB 1: Profiling + GoF + Quick Runner ----------
with TAB1:
    st.subheader('📈 Profiling — Visuals + GoF + Quick Runner')
    c1,c2 = st.columns(2)
    with c1:
    ...
    if num_col:
        ...
        if len(s)==0:
            ...
            st.markdown('**Bảng chỉ số chi tiết (Numeric)**')
            ...
            if HAS_PLOTLY:
                ...
                with gA:
                    ...
                    st_plotly(fig1); register_fig('Profiling', f'{num_col} — Histogram+KDE', fig1, 'Hình dạng phân phối & đuôi; KDE có thể bị tắt nếu n rất lớn.')
                    st.caption('**Ý nghĩa**: Nhìn shape, lệch, đa đỉnh; KDE giúp mượt mật độ.')
                with gB:
                    ...
                    st.caption('**Ý nghĩa**: Trung vị & IQR; các điểm bật ra là ứng viên ngoại lệ.')
                gC,gD = st.columns(2)
                with gC:
                    try:
            ...
            if len(num_cols)>=2: recs.append('Correlation với biến numeric khác.')
            st.write('\n'.join([f'- {x}' for x in recs]) if recs else '- Không có đề xuất đặc biệt.')

            # Quick Runner
            with st.expander('⚡ Quick Runner (run recommended tests here)'):
                        ...
                        if '1D' in qtype:
                            ...
                            if not r: st.error('Cannot extract first digit.')
                                ...
                                if HAS_PLOTLY:
                                    ...
                                    fig.update_layout(title='Benford 1D — Obs vs Exp', height=340)
                                    st_plotly(fig); register_fig('Benford 1D','Benford 1D — Obs vs Exp (Quick)', fig, 'Benford 1D.')
                                st.dataframe(var, width='stretch', height=220)
                        else:
                            r = benford_2d(s)
                            if not r: st.error('Cannot extract first‑two digits.')
                            else:
                                tb, var = r['table'], r['variance']
                                if HAS_PLOTLY:
                                    fig = go.Figure(); fig.add_trace(go.Bar(x=tb['digit'], y=tb['observed_p'], name='Observed'))
                                    fig.add_trace(go.Scatter(x=tb['digit'], y=tb['expected_p'], name='Expected', mode='lines', line=dict(color='#F6AE2D')))
                                    fig.update_layout(title='Benford 2D — Obs vs Exp', height=340)
                                    st_plotly(fig); register_fig('Benford 2D','Benford 2D — Obs vs Exp (Quick)', fig, 'Benford 2D.')
                                st.dataframe(var, width='stretch', height=220)
                    ...
                    if len(cat_cols)==0:
                        ...
                        if st.button('Run now', key='qr_anova'):
                            if grp not in df.columns or num_col not in df.columns:
                                st.error('Cột đã chọn không còn tồn tại trong dữ liệu hiện tại.');
                            else:
                            ...
                            if y2 not in df.columns or num_col not in df.columns:
                                ...
                                if len(sub)<3: st.error('Không đủ dữ liệu sau khi drop NA.')
                                    ...
                                    if HAS_PLOTLY:
                                        ...
                                        st_plotly(fig); register_fig('Tests', f'{num_col} vs {y2} (Quick Corr)', fig, 'OLS trend.')
...
with TAB2:
    ...
    with cA:
        ...
        if (dt_col is None) or (dt_col not in df.columns):
            st.warning('Chọn cột thời gian hợp lệ.');
        else:
    ...
    with cB:
        if len(num_cols)>=2 and HAS_PLOTLY:
            ...
            st_plotly(figH); register_fig('Correlation', 'Correlation heatmap', figH, 'Linear correlation among numeric vars.')
            st.caption('**Ý nghĩa**: |r| cao → mối liên hệ mạnh; hỗ trợ chọn biến cho Regression.')
        else:
...
with TAB4:
    ...
    if typ=='anova':
        ...
        if len(num_cols)==0 or len(cat_cols)==0:
            ...
            if (y is None) or (g is None) or (y not in df.columns) or (g not in df.columns):
                st.warning('Chọn cột Y & Group hợp lệ.');
            else:
                sub = df[[y,g]].dropna()
                if sub[g].nunique()<2:
        ...
        if (flag_col is None) or (g is None) or (flag_col not in df.columns) or (g not in df.columns):
        ...
        if (a is None) or (b is None) or (a==b) or (a not in df.columns) or (b not in df.columns):
        ...
        st.info('**Khi dùng**: Tương quan tuyến tính giữa hai biến numeric; lưu ý outliers.')
        if len(num_cols)<2:
            ...
            if (x is None) or (y is None) or (x==y) or (x not in df.columns) or (y not in df.columns):
                ...
                if len(sub)<3:
                    ...
                    if HAS_PLOTLY:
                        ...
                        st_plotly(fig); register_fig('Tests', f'{x} vs {y} (corr)', fig, 'Linear association with OLS trend.')
    ...
    if st.button('🔎 Scan Flags'):
        ...
        for title, obj in visuals:
            if HAS_PLOTLY and not isinstance(obj, pd.DataFrame):
                # already registered in compute if plotly
                pass
...
with TAB7:
    ...
    with cA:
        ...
        with st.expander('📋 Hướng dẫn nhanh (logic)'):
            st.markdown('''
- **Population Analysis**: đọc mean/SE/CI, shape/tails/normality; xác nhận Histogram+KDE/Box/ECDF/QQ.
...
- **Quan hệ biến** → **Correlation**; nếu dự báo/giải thích → **Regression**.
''')

    with cB:
        st.subheader('🧾 Export (Plotly snapshots)')
        incl = st.multiselect('Include sections', ['Population','Profiling','Trend','Correlation','Benford 1D','Benford 2D','Tests','Regression','Fraud Flags'],
                              default=['Population','Profiling','Benford 1D','Benford 2D','Tests'])
