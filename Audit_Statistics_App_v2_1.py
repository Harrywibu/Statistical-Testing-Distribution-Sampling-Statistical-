...
if fname.lower().endswith('.csv'): 
 ...
 with st.expander('📁 Select sheet & header (XLSX)', expanded=True): 
  ...
  st.caption(f'Columns: {len(headers)} \n SHA12={SS["sha12"]}') 
...
with TAB1: 
  ...
  if len(s)==0: 
   ...
   if HAS_PLOTLY: 
    ...
    with g1_col: 
     ...
     st.plotly_chart(fig1, width='stretch', config={'displaylogo': False}); register_fig('Profiling', f'{num_col} — Histogram+KDE', fig1, 'Shape (tail/skew). KDE suppressed for very large n.') 
    with g2_col: 
     fig2 = px.box(pd.DataFrame({num_col:s}), x=num_col, points='outliers', title=f'{num_col} — Box') 
     st.plotly_chart(fig2, width='stretch', config={'displaylogo': False}); register_fig('Profiling', f'{num_col} — Box', fig2, 'Median/IQR; outliers.') 
    g3_col, g4_col = st.columns(2) 
    with g3_col: 
     try: 
      fig3 = px.ecdf(s, title=f'{num_col} — ECDF') 
      st.plotly_chart(fig3, width='stretch', config={'displaylogo': False}); register_fig('Profiling', f'{num_col} — ECDF', fig3, 'Cumulative probability P(X≤x).') 
     except Exception: 
    ...
    with g4_col: 
     try: 
      ...
      st.plotly_chart(fig4, width='stretch', config={'displaylogo': False}); register_fig('Profiling', f'{num_col} — QQ Normal', fig4, 'Deviation from normal assumption.') 
     except Exception: 
    ...
    if SS['advanced_visuals']: 
     ...
     with g5_col: 
      ...
      st.plotly_chart(figv, width='stretch', config={'displaylogo': False}); register_fig('Profiling', f'{num_col} — Violin', figv, 'Density shape + box overlay.') 
     with g6_col: 
      # Lorenz & Gini 
      ...
      st.plotly_chart(figL, width='stretch', config={'displaylogo': False}); register_fig('Profiling', f'{num_col} — Lorenz', figL, 'Value concentration (inequality).') 
 ...
 if cat_col: 
  ...
  if HAS_PLOTLY: 
   ...
   st.plotly_chart(figc, width='stretch', config={'displaylogo': False}); register_fig('Profiling', f'{cat_col} — Top {int(topn)}', figc, 'Dominant categories.') 

 # Data Quality quick view 
 with st.expander('🧪 Data Quality (DQ)', expanded=False): 
  rep, n_dupes = quality_report(df); st.write(f'Duplicate rows: **{n_dupes}**'); st.dataframe(rep, width='stretch', height=260) 

# ---------- Tab 2: Trend & Correlation ----------
with TAB2: 
 st.subheader('📈 Trend analysis & 🔗 Correlation') 
 dt_candidates = [c for c in df.columns if pd.api.types.is_datetime64_any_dtype(df[c]) or re.search(r"date\ntime", str(c), re.IGNORECASE)] 
 cA, cB = st.columns(2) 
 with cA: 
  ...
  if HAS_PLOTLY: 
   ...
   if not sub.empty: 
    ...
    st.plotly_chart(figt, width='stretch', config={'displaylogo': False}); register_fig('Trend', f'{num_col2} — Trend ({freq})', figt, 'Aggregated time series with rolling mean.') 
 with cB: 
  if len(num_cols)>=2 and HAS_PLOTLY: 
   ...
   st.plotly_chart(figH, width='stretch', config={'displaylogo': False}); register_fig('Correlation', 'Correlation heatmap', figH, 'Linear correlation among numeric variables.') 
  else: 
   ...
   if st.button('Run Benford 1D'): 
    ...
    if not res1: st.error('Cannot extract first digit.') 
     ...
     if HAS_PLOTLY: 
      ...
      st.plotly_chart(fig1, width='stretch', config={'displaylogo': False}); register_fig('Benford 1D', 'Benford 1D — Obs vs Exp', fig1, 'First digit distribution vs Benford.') 
     st.markdown('**Variance (counts)**'); st.dataframe(var, width='stretch', height=240) 
   ...
   if st.button('Run Benford 2D'): 
    ...
    if not res2: st.error('Cannot extract first‑two digits.') 
     ...
     if HAS_PLOTLY: 
      ...
      st.plotly_chart(fig2, width='stretch', config={'displaylogo': False}); register_fig('Benford 2D', 'Benford 2D — Obs vs Exp', fig2, 'First‑two digits distribution vs Benford.') 
     st.markdown('**Variance (counts)**'); st.dataframe(var2, width='stretch', height=240) 
 ...
 if typ=='anova': 
  if len(num_cols)==0 or len(cat_cols)==0: st.warning('Need a numeric target and a categorical group.') 
   ...
   if sub[g].nunique()<2: st.warning('Select a group column with ≥2 levels.') 
    ...
    if HAS_PLOTLY: 
     fig = px.box(sub, x=g, y=y, color=g, title=f'{y} by {g}'); st.plotly_chart(fig, width='stretch', config={'displaylogo': False}); register_fig('Tests', f'{y} by {g} (ANOVA)', fig, 'Group mean comparison.') 
    ...
    if p<0.05 and HAS_SM: 
     try: 
      ...
      st.markdown('**Post‑hoc: Tukey HSD**'); st.dataframe(df_tuk, width='stretch', height=240) 
     except Exception: 
      pass 
 elif typ=='prop': 
  ...
  if sub.empty: st.warning('Provide a valid flag & group.') 
   ...
   if HAS_PLOTLY: 
    fig = px.imshow(ct, text_auto=True, aspect='auto', color_continuous_scale='Blues', title='Proportion by group'); st.plotly_chart(fig, width='stretch', config={'displaylogo': False}); register_fig('Tests', 'Proportion by group', fig, 'Compliance rate across groups.') 
  ...
  if sub.empty: st.warning('Pick two categorical columns with sufficient data.') 
   ...
   if HAS_PLOTLY: 
    fig = px.imshow(ct, text_auto=True, aspect='auto', color_continuous_scale='Reds', title='Contingency table'); st.plotly_chart(fig, width='stretch', config={'displaylogo': False}); register_fig('Tests', 'Contingency χ²', fig, 'Dependence strength via Cramér V.') 
  ...
  if len(num_cols)<2: st.warning('Need two numeric columns.') 
   ...
   if len(sub)<3: st.warning('Not enough data after dropping NA.') 
    ...
    if HAS_PLOTLY: 
     fig = px.scatter(sub, x=x, y=y, trendline='ols', title=f'{x} vs {y}'); st.plotly_chart(fig, width='stretch', config={'displaylogo': False}); register_fig('Tests', f'{x} vs {y} (corr)', fig, 'Linear association with OLS trend.') 
    ...
    if st.button('Run Linear Regression', key='btn_lin'): 
     ...
     if len(sub) < (len(X_t)+5): 
      ...
      if HAS_PLOTLY: 
       ...
       st.plotly_chart(fig1, width='stretch', config={'displaylogo': False}); register_fig('Regression', 'Residuals vs Fitted', fig1, 'Mean‑zero & homoscedastic residuals desired.') 
     ...
     if len(sub) < (len(Xb)+10): 
      ...
      try: 
       ...
       if HAS_PLOTLY: 
        ...
        st.plotly_chart(fig, width='stretch', config={'displaylogo': False}); register_fig('Regression', 'ROC Curve', fig, 'Discrimination power of the model.') 
      except Exception as e: 
...
with TAB7: 
 ...
 with cA: 
  ...
  st.dataframe(pd.DataFrame(signals) if signals else pd.DataFrame([{'status':'No strong risk signals'}]), width='stretch', height=300) 
